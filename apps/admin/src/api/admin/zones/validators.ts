import { z } from "zod";
import {
  BackendUpdateSlotItemSchema,
  BaseSlotDefinitionSchema,
  FrontendSlotItemSchema,
} from "./validation/slot-validation";
...
export const AdminCreateZone = z.object({
  ...
  postcodes: z
    .array(z.string().regex(POSTCODE_REGEX, POSTCODE_ERROR_MESSAGE))
...
const BaseInstantPromiseSchema = z.object({
  ...
  pickup_lead_minutes: z
    .number()
    .min(0, "Pickup lead minutes must be non-negative")
    .default(0),
  return_lead_minutes: z
    .number()
    .min(0, "Return lead minutes must be non-negative")
    .default(0),
  is_active: z.boolean().default(true),
});
