...
import {
  generateProductVariantInventoryFeedBackgroundWorkflow,
} from "../../../../workflows/product-feed/workflows/generate-product-variant-inventory-feed-background"
import type { AdminGetProductVariantFeedParamsType } from "../product-variants/validators"
...
export async function GET(
  ...
  const { result } = await generateProductVariantInventoryFeedBackgroundWorkflow(
    req.scope
  ).run({
    input: {
      transaction_id,
