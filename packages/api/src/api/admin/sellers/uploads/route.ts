...
export const POST = async (
  ...
  if (!seller_name) {
    return res.status(400).json({
      url: '',
      file_path: '',
      message: 'seller_name is required'
    })
  }

  // Validate that either file or base64_content is provided
  ...
  try {
    ...
    // Handle base64 content (from JSON)
    if (base64_content) {
      // Extract base64 data (remove data:type;base64, prefix if present)
      const base64Data = base64_content.includes(',') 
        ? base64_content.split(',')[1] 
        : base64_content
      
      // Extract mime type from base64 prefix if available
      ...
      // Get file extension from mime type or file_name
    ...
    // Handle multipart file upload
      ...
      fileBuffer = Buffer.isBuffer(file.buffer) 
        ? file.buffer 
    ...
    const sellerName = seller_name.replace(/\s+/g, '').replace(/[^a-zA-Z0-9]/g, '') // Clean seller name

    let fileNameWithPath: string

    if (type === 'member') {
      // Member photo path: /documents/sellers/{yyyy-mm}/member/{seller_name}_member_{fullDate}.ext
      fileNameWithPath = `documents/sellers/${yearMonth}/member/${sellerName}_member_${fullDate}.${fileExtension}`
    } else if (doc_type) {
      // Validate doc_type for KYC documents
      ...
      // KYC document path: /documents/sellers/{yyyy-mm}/{doc_type}/{seller_name}_{doc_type}_{fullDate}.ext
      ...
      return res.status(400).json({
        url: '',
        file_path: '',
        message: 'Either type="member" or doc_type must be provided'
      })
    }
