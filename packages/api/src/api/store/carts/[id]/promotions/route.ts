...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (cartForMetadata?.metadata) {
      ...
      const rejectedCodesRaw = (metadata as Record<string, unknown>)?.rejected_auto_promo_codes
      const rejectedCodes: string[] = Array.isArray(rejectedCodesRaw)
        ? rejectedCodesRaw.filter((code): code is string => typeof code === "string")
        : []
    ...
    if (newCodeRules?.override_existing === true) {
      ...
      const existingPromotions = cartWithPromos?.promotions ?? []
      if (existingPromotions.length > 0) {
        // Check each existing promotion's override_existing flag
        for (const existingPromo of existingPromotions) {
          const existingCode = existingPromo?.code
          if (!existingCode) {
            continue
          }

          const existingRules = await getPromotionRulesWithCache(existingCode, req.scope)
  ...
  try {
    ...
    if (removedAutoPromoCodes.length > 0) {
      ...
      if (cartForRejection) {
        ...
        const existingRejectedCodesRaw = (metadata as Record<string, unknown>)?.rejected_auto_promo_codes
        const existingRejectedCodes: string[] = Array.isArray(existingRejectedCodesRaw)
          ? existingRejectedCodesRaw.filter((code): code is string => typeof code === "string")
          : []
