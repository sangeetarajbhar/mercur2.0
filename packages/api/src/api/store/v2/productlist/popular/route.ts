 ...
 * Returns popular search keywords sorted by search popularity.
 * 
 * Query Parameters:
...
export const GET = async (
  ...
  try {
    ...
    const result = await searchService.getPopularSearches(req.scope, params)
    res.json(result)
  } catch (error: unknown) {
    console.error('[Popular Searches] Error fetching popular searches:', error instanceof Error ? error.message : String(error))
    res.status(500).json({
