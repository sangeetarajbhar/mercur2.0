...
const FINAL_STATUSES = [...ORIGINAL_STATUSES, ...ADDITIONAL_STATUSES, ...NEW_STATUSES]
...
export class Migration20251222120000 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(`ALTER TABLE "order" ALTER COLUMN "status" SET DEFAULT 'PAYMENT_PENDING';`)
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`
      UPDATE "order"
      SET "status" = 'pending'
      WHERE "status" IN (${NEW_STATUSES.map((status) => `'${status}'`).join(', ')});
    `)

    this.addSql(`ALTER TYPE "${ORDER_STATUS_ENUM}" RENAME TO "${ORDER_STATUS_ENUM}_new";`)

    const STATUSES_WITHOUT_NEW = [...ORIGINAL_STATUSES, ...ADDITIONAL_STATUSES]

    this.addSql(`
      CREATE TYPE "${ORDER_STATUS_ENUM}" AS ENUM (
        ${STATUSES_WITHOUT_NEW.map((status) => `'${status}'`).join(', ')}
      );
    `)
