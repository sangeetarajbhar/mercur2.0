...
export class Migration20251222125506 extends Migration {

  override async up(): Promise<void> {
    // Add PAYMENT_PENDING to enum if it doesn't exist
      ...
      BEGIN
        ...
        IF enum_type_oid IS NOT NULL THEN
          IF NOT EXISTS (
            ...
            WHERE enumlabel = '${OrderLineItemStatus.PAYMENT_PENDING}' 
            AND enumtypid = enum_type_oid
          ) THEN
            -- Add the enum value using EXECUTE format (consistent with existing migrations)
            EXECUTE format('ALTER TYPE order_line_item_status_enum ADD VALUE %L', '${OrderLineItemStatus.PAYMENT_PENDING}');
          END IF;
        END IF;
      END $$;
    `);
