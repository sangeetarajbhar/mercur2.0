...
export class Migration20251222152904 extends Migration {
    ...
    this.addSql(`
      ...
      BEGIN
        ...
        SELECT EXISTS (
          ...
          WHERE t.typname = 'order_line_item_status_enum'
          AND e.enumlabel = '${OrderLineItemStatus.PAYMENT_PENDING}'
        ) INTO enum_exists;
        
        -- Only proceed if enum value exists
        IF enum_exists THEN
          -- Get current default value (may be 'NEW' or 'NEW'::order_line_item_status_enum format)
          ...
          WHERE c.relname = 'order_line_item_extension'
          ...
          IF current_default IS NULL 
             ...
             OR current_default NOT LIKE '''${OrderLineItemStatus.PAYMENT_PENDING}''%' THEN
            BEGIN
              ALTER TABLE "order_line_item_extension" 
              ALTER COLUMN "status" SET DEFAULT '${OrderLineItemStatus.PAYMENT_PENDING}';
            EXCEPTION
              WHEN OTHERS THEN
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`
      ALTER TABLE "order_line_item_extension" 
      ALTER COLUMN "status" SET DEFAULT '${OrderLineItemStatus.NEW}';
    `);
  }
