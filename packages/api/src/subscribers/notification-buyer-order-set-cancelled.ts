...
export default async function orderSetCancelledHandler({
  event,
  container
}: SubscriberArgs<{ id: string; canceled_by?: string; canceled_at?: string }>) {
  ...
  moEngageService.sendNotifications({
    alertName: MoEngageAlertName.ORDER_SET_CANCELLED,
    recipient: {
...
export const config: SubscriberConfig = {
  event: 'order-set-cancelled',
  context: {
    subscriberId: 'notification-buyer-order-set-cancelled-handler'
  }
}
