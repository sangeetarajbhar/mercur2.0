...
export default async function returnCancelledSQSHandler({
  event,
  container
  ...
  if (!return_id) {
    console.warn('[Return Cancelled SQS] Missing return_id in event data')
    return
  }
  ...
  try {
    ...
    if (!returns || returns.length === 0) {
      console.warn(`[Return Cancelled SQS] No return found for return_id: ${return_id}`)
      return
    }
    ...
    if (!orderId) {
      console.warn(`[Return Cancelled SQS] No order_id found for return_id: ${return_id}`)
      return
    }
    ...
    if (!sellerOrderLinks || sellerOrderLinks.length === 0) {
      console.warn(`[Return Cancelled SQS] No seller found for order_id: ${orderId}`)
      return
    }
    ...
    if (!orderExtraDetails || orderExtraDetails.length === 0) {
      console.warn(`[Return Cancelled SQS] No order_extra_detail found for order_id: ${orderId}`)
      return
    }
    ...
    console.log(`[Return Cancelled SQS] Successfully sent message for return_id: ${return_id}`)
  } catch (error) {
    console.error('[Return Cancelled SQS] Error processing return_cancelled event:', error instanceof Error ? error.message : error)
  }
}
...
export const config: SubscriberConfig = {
  ...
  context: {
    subscriberId: 'return-cancelled-sqs-handler'
  }
}
