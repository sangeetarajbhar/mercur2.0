...
async function getInventoryByVariantIdSellerId(
  ...
  const inventoryResult = await knex.raw(`
    ...
    JOIN
      stock_location_stock_location_seller_seller slss ON (il.location_id = slss.stock_location_id AND slss.deleted_at IS NULL)
    WHERE
      pv.id = '${variantId}' AND ssii.seller_id = '${sellerId}' AND slss.seller_id = '${sellerId}' AND (il.stocked_quantity - il.reserved_quantity > 0)
        AND slsle.stock_location_id IN (${formattedIds}) AND slss.stock_location_id IN (${formattedIds})
  `)
...
export const mapCartItemsToLocationStep = createStep(
  ...
  async (
    ...
    const zone = await fetchZoneByPincode(postal_code)
