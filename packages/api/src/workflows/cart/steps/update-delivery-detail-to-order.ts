...
import { validateDateIsTodayOrTomorrow, combineDateAndTime, parseDateString, validateDateTimeIsInFuture } from './helpers/delivery-validation-utils'
...
export const updateDeliveryDetailToOrderStep = createStep(
  ...
  async (
    ...
    try {
      ...
      const deliveryDate =
        cartDeliveryDetail.delivery_date instanceof Date
          ? cartDeliveryDetail.delivery_date
          : parseDateString(cartDeliveryDetail.delivery_date)

      // Validate end time is in the future (in IST)
      const endDateTime = combineDateAndTime(deliveryDate, cartDeliveryDetail.end_time)
