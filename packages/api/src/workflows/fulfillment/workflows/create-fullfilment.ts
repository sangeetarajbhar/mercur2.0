import {
  ...
  function prepareFulfillmentData({
    ...
    const fulfillmentItems = fulfillableItems
      .map((i) => {
        ...
        // Simplified: Create fulfillment items with inventory_item_id but without inventory handling
        return {
          line_item_id: i.id,
          inventory_item_id: orderItem.variant?.inventory_items?.[0]?.inventory?.id ?? null,
          quantity: i.quantity,
  ...
  export const createOrderFulfillmentWorkflowId = "create-order-fulfillment-custom"
  /**
  ...
  export const createOrderFulfillmentWorkflow = createWorkflow(
    ...
    (input: WorkflowData<CreateOrderFulfillmentWorkflowInput>) => {
      const { data: order }: any = useQueryGraphStep({
        entity: "order",
      ...
      createFulfillmentValidateOrder({ order: order as any, inputItems: input.items })
      ...
      const shippingOptionId = transform(
        { order: order as any, input: input as any },
        ({ order, input }: any) => {
          return (
