import {
  ...
  export function throwIfOrderIsCancelled({ order }: { order: OrderDTO }) {
    ...
    // @ts-ignore - OrderStatus.CANCELED and 'CANCELLED' are valid status values but not in the type definition
    if (order.status === OrderStatus.CANCELED || order.status === 'CANCELLED') {
