/**
 * Shopify ID Mapper Service
 * Single Responsibility: Maps SKUs to Shopify product and variant IDs
...
export interface IShopifyIdMapper {
  mapSkusToShopifyIds(skus: Set<string>): Promise<Map<string, { productId: string; variantId: string }>>
}

export class ShopifyIdMapper implements IShopifyIdMapper {
  constructor(private query: IQueryService) {}

  async mapSkusToShopifyIds(skus: Set<string>): Promise<Map<string, { productId: string; variantId: string }>> {
