...
const normalizeProductForImport = (
  ...
  Object.entries(rawProduct).forEach(([key, value]) => {
    ...
    // Store image URLs in metadata instead of adding to product images
    // Images will be processed separately after product creation
    if (normalizedKey.startsWith("product_image_")) {
      // Store in metadata for later processing
      if (!response["metadata"]) {
        response["metadata"] = {}
      }
      if (!response["metadata"]["_pending_images"]) {
        response["metadata"]["_pending_images"] = []
      }
      response["metadata"]["_pending_images"].push(normalizedValue)
      return
    }
