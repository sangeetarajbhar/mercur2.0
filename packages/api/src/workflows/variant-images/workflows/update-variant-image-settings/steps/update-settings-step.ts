...
export const updateSettingsStep = createStep('update-settings-step', async (input: UpdateVariantImagesSettingsInput, { container }) => {
  ...
  const _productSettings = await SettingsModule
    .retrieveVariantImagesSettings(input.product_id)
    .catch(() => undefined);

  if (!_productSettings) {
    return new StepResponse(
      await SettingsModule.createVariantImagesSettings(input)
    );
  }

  return new StepResponse(
    await SettingsModule.updateVariantImagesSettings(input)
  );
});
